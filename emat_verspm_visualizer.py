...
class VERSPMModel(FilesCoreModel):
	...
	def __init__(self, db=None, db_filename="verspm.db", scope=None):
		...
		#if scope is None:
		#	scope = Scope(join_norm(this_directory, 'verspm-emat-files', "verspm-scope.yml"))
		#	for i in ['scope']:
		#		shutil.copy2(
		#			join_norm(this_directory, 'verspm-emat-files', f"verspm-{i}.yml"),
		#			join_norm(cwd, f"verspm-{i}.yml"),
		...
		# 	runConfig_ls <-  yaml::read_yaml(config_file)
		# 	runConfig_ls[["Years"]] <-  c('{self.config["model_year"]}')
		# 	runConfig_ls[["LoadModel"]] <-  '{self.config['base_model']}'
		# 	# runConfig_ls <-  list(
	...
	def _manipulate_marea_transit_service(self, params):
		...
		Args:
			params (dict):
				...
				exogenous uncertainties and policy levers.
		"""
		marea_df = pd.read_csv(scenario_input(os.path.join('TS', 'RTP23', 'marea_transit_service.csv')))
		bzone_df = pd.read_csv(scenario_input(os.path.join('TS', 'RTP23', 'bzone_transit_service.csv')))
		model_year = int(self.config["model_year"])
		# Update marea transit revenue file
		for marea_col in marea_df.columns:
			if marea_col not in ['Geo', 'Year']:
				marea_df.loc[marea_df.Year==model_year,marea_col] = (1+params['TRANSITSERVICE'])*marea_df.loc[marea_df.Year==model_year,marea_col]
		# Update bzone transit revenue file
		for bzone_col in bzone_df.columns:
			if bzone_col not in ['Geo', 'Year']:
				bzone_df.loc[bzone_df.Year==model_year,bzone_col] = (1+(params['TRANSITSERVICE']/2))*bzone_df.loc[bzone_df.Year==model_year,bzone_col]
		# Write out files
		marea_df.to_csv(join_norm(self.resolved_model_path, 'inputs', 'marea_transit_service.csv'), index=False)
		bzone_df.to_csv(join_norm(self.resolved_model_path, 'inputs', 'bzone_transit_service.csv'), index=False)
		return True
	...
	def post_process(self, params=None, measure_names=None, output_path=None):
		...
		if not os.path.exists(join_norm(self.local_directory, self.modelname, "extract_outputs_new.R")):
			shutil.copy2(
					join_norm(this_directory, "extract_outputs_new.R"),
					join_norm(self.local_directory, self.modelname, "extract_outputs_new.R"),
				)
		...
		self.postprocess_results = subprocess.run(
			#[cmd, "extract_outputs_new.R", cwd2],
			#[cmd, "extract_outputs_new.R", self.config["model_year"]],
			[cmd, "extract_outputs_new.R"],
			cwd=cwd2,
			capture_output=True,
		)
